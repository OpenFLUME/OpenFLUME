...
describe("alignTraces: common-domain alignment", () => {
  ...
  it("returns an empty alignment (metrics undefined) when supports do not overlap", () => {
    const a = alignTraces(
      sample(uniform(0, 10, 1), (x) => x),
      sample(uniform(20, 30, 1), (x) => x),
    );
...
describe("run-level aggregation: runs are the independent unit", () => {
  ...
  it("runs/stations with undefined metrics (no overlap) are skipped, not zero-filled", () => {
    ...
    expect(
      aggregateRunLevelMetrics([
        run,
        { nStations: 0, nSamples: 0, weightSum: 0 },
      ]).nRuns,
    ).toBe(1);
  });
...
describe("feature observables on a known synthetic quench curve", () => {
  ...
  it("smooth crossings land exactly on the piecewise-linear curve", () => {
    ...
    expect(feats.crossing150KS).toEqual({
      available: true,
      value: expect.closeTo(50 + 150 / 11, 9),
    });
    // knee at 95 K: t = 50 + 205/11 = 68.6364
    expect(feats.kneeTimeS).toEqual({
      available: true,
      value: expect.closeTo(50 + 205 / 11, 9),
    });
  ...
  it("onset: belowThreshold (290 K) and dropFromStart (5 K) conventions", () => {
    const fFixed = extractTraceFeatures(traceQ, {
      onset: { mode: "belowThreshold", thresholdK: 290 },
    });
    // 300 − 11(t−50) = 290 → t = 50 + 10/11
    expect(fFixed.onsetTimeS).toEqual({
      available: true,
      value: expect.closeTo(50 + 10 / 11, 9),
    });
    expect(fFixed.onsetThresholdK).toBe(290);
    const fDrop = extractTraceFeatures(traceQ, {
      onset: { mode: "dropFromStart", dropK: 5 },
    });
    // 295 K → t = 50 + 5/11
    expect(fDrop.onsetTimeS).toEqual({
      available: true,
      value: expect.closeTo(50 + 5 / 11, 9),
    });
    expect(fDrop.onsetThresholdK).toBe(295);
  });

  it("peak cooling rate recovers the exact −11 K/s slope of the drop", () => {
    const feats = extractTraceFeatures(traceQ, {
      kneeThresholdK: 95,
      rateHalfWindowS: 2,
    });
  ...
  it("coldTailUsable=false ⇒ cold-side features unavailable, mid-front retained", () => {
    ...
    const feats = extractTraceFeatures(trTrunc, {
      kneeThresholdK: 95,
    ...
    for (const f of [
      ...
      feats.kneeTimeS,
      feats.drop150to50S,
      feats.plateauFractionPreKnee,
    ]) {
      expect(f.available).toBe(false);
    }
    if (!feats.kneeTimeS.available)
      expect(feats.kneeTimeS.reason).toMatch(/cold tail flagged unusable/);
  });
  ...
  it("a crossing in the FINAL sample interval is unavailable (truncation boundary guard)", () => {
    ...
    const feats = extractTraceFeatures(
      { timesS: [0, 1, 2], valuesK: [300, 200, 90] },
      {},
    );
...
describe("front-arrival ordering", () => {
  ...
  const mkTrace = (delayS: number) =>
    sample(uniform(0, 100, 1), (x) => 300 - Math.max(0, x - delayS) * 5);
...
describe("corpus integration smoke checks (real data through the objectives)", () => {
  ...
  it("QC-gated features on the audit-flagged trusted traces", () => {
    ...
    const feats = extractTraceFeatures(
      ...
      {
        kneeThresholdK: 86.1 + 15,
        coldTailUsable: fig10stn4.qc.coldTailUsable,
      },
    );
    ...
    const feats11 = extractTraceFeatures(
      ...
      {
        kneeThresholdK: 89.5 + 15,
        coldTailUsable: fig11stn1.qc.coldTailUsable,
      },
    );
    ...
    const feats12 = extractTraceFeatures(
      ...
      {
        kneeThresholdK: 96.3 + 15,
        coldTailUsable: fig12stn1.qc.coldTailUsable,
      },
    );
