...
describe("TRACE_BASELINE_SOLVE_PLAN: pre-registered discretization per trusted run", () => {
  it("covers exactly the 4 trusted saturated corpus runs", () => {
    ...
    const trustedIds = NBS_TRUSTED_SATURATED_TRACE_RUNS.map(
      (r) => r.runId,
    ).sort();
          ...
          timesS.map((tt) =>
            ...
            tt <= t.timesS[0]
              ...
              : tt >= t.timesS[t.timesS.length - 1]
                ...
                : t.wallTempsK[
                    Math.max(0, t.timesS.findIndex((x) => x >= tt) - 1)
                  ],
          ),
        ),
      };
...
describe("feature conventions", () => {
  it("onset: belowThreshold(290) when the record starts above 290 K, else dropFromStart(5)", () => {
    expect(
      onsetOptionsFor({ timesS: [0, 1], valuesK: [300, 250] }).convention,
    ).toBe("belowThreshold290");
    expect(
      onsetOptionsFor({ timesS: [0, 1], valuesK: [285, 250] }).convention,
    ).toBe("dropFromStart5");
  });
  ...
  it("the run rate half-window is the max over model AND data adaptive windows", () => {
    ...
    const fine = {
      timesS: [0, 0.5, 1, 1.5, 2, 2.5, 3],
      valuesK: [300, 290, 280, 270, 260, 250, 240],
    };
  ...
  it("compareStationTrace wires QC weight and both feature sets through", () => {
    ...
    const model = {
      timesS: data.timesS,
      valuesK: data.wallTempsK.map((v) => v + 5),
    };
