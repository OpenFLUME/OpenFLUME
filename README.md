...
| `junctions`                           | `JunctionConfig[]`                                                               | no                       | Steady CEA-coupled reacting junctions; requires `kineticEnergy`, cannot be combined with `species`.                                                                                                                                                                                                                                  |
...
**Steady combustion architecture.** `config.junctions` provides a separate,
monolithically coupled CEA chamber closure for steady + `kineticEnergy` solves.
It currently supports LOX/RP-1 and LOX/CH₄, requires a named ideal-gas product
fluid, and cannot be combined with species transport. Product composition is
frozen downstream and the tables assume standard-state reactant injection. See
...
- [x] Reacting flows — transient ideal-gas species transport with stiff Arrhenius chemistry and steady CEA-coupled reacting junctions (see [Species Transport and Reacting Flows](#species-transport-and-reacting-flows))
