...
- **Species Transport & Reacting Flows** – optional multi-species advective transport on transient `idealGas` networks with node-local stiff-chemistry sub-stepping (BDF1 + dense Newton). A separate steady reacting-junction model couples LOX/RP-1 or LOX/CH₄ feeds to NASA CEA chamber tables inside the Newton system; it requires `kineticEnergy` and freezes product composition downstream. See [`docs/combustion.md`](docs/combustion.md).
...
round-trips through the `.fn` file and never marks results stale). Rows that
need attention carry an icon — an amber triangle for a readiness check, a red
circle for a validation error; healthy rows carry nothing.

`Cmd`/`Ctrl`+`K` opens a command palette that can run the model, place
elements, switch views, and jump to any element by id.
...
- **Branches** – edit the Newton warm start (**Initial flow guess**) and component-specific fields (length, diameter, roughness, area, Cd, K, pump curve, etc.). Pipes choose between the Swamee–Jain **Correlation** and a **Constant f**, and can be given a **Tapered outlet** (`diameterOut`) for nozzle segments; heated pipes select a **Boiling model**; a `customResistance` edits its K(Re) curve as a point grid, promotes a constant K to a table, or collapses a table back to a constant.
- **Real-fluid nodes** – choose **Temperature** or **Vapour quality** as the state variable (mutually exclusive, quality only where a saturation dome exists). Boundary nodes gain a **Cryogenic front inlet** marker once a conductor opts into front transport, and every node gains per-species **Composition** fields once the network declares species.
...
- **Solver** – **Steady** or **Transient**, with tolerance, max iterations, and relaxation. Transient adds **Fixed dt** or **Adaptive** (min/max/initial dt, relative tolerance, safety factor, and the absolute pressure/temperature floors) plus end time. Internal nodes must have a non-zero `volume`. An **Advanced numerics** disclosure holds the Newton strategy: steady solver (PTC / direct), globalization (trust region / line search), Jacobian (hybrid / FD), and the experimental re-certify-after-coupling flag.
- **Physics** – **Momentum flux** and **Kinetic energy** (together, the quasi-1-D compressible formulation), the **Momentum-flux scheme** (limited upwind / central), and the default-on **Second-law admissibility audit**. A derived summary line names the formulation the current flags select, and each control is disabled with an explanation where it has no effect. A **Closure calibration** disclosure exposes the Dittus–Boelter, Miropolskii, and Swamee–Jain constants plus `solidCpScale`; blank means the published value.
...
- **Species** – enable multi-species transport, edit the species roster (names, molecular weights, and optional cp / formation enthalpy / viscosity columns), and build Arrhenius reactions with per-species stoichiometry. Requires a single ideal-gas continuum.
- **Units** – display preset (SI, Metric engineering, US customary) and per-quantity preferences.
- **Extensibility** – JSON editors for registers, logic rules, and PID controllers.
...
**The x axis** is chosen at the axis — the label under the chart is the control — and is Time (transient only), Station along a flow path, Position X/Y/Z, or Element order. It decides what a channel means:

- On **Time** a channel is a _series_ — three node pressures draw three lines.
- On any other axis a channel is a _point_ — three node pressures draw one line across the network, because what varies along the axis is the element, not the sample. Channels group into a line per quantity.

So node pressures against **Station** is the hydraulic grade line. Flow direction is taken from the solved mass-flow signs rather than declared in the model, so the path selector offers every route the result supports (one per outlet of a tee, per stream of a counterflow pair, or a closed loop). Per-span quantities such as branch ΔP draw as stairs. The axis degrades honestly: real metres when every component on the path has a length, an index that says so when it does not, and an element with no coordinate is left out and named rather than placed at zero.

**Channels** come from a picker whose **Plotted** block states what the plot draws (value and an × per channel), over a list of everything else the result carries. One line of controls narrows it: a search box, a sort menu (by quantity, the default, or by element) and a filter menu (fluid nodes, branches, solids, conductors, each with its canvas glyph). Each row shows its current value; clicking it adds it to the plot and selects that element in the model. A preset control fills the plot with a whole family in one go.

Deterministic **findings** appear under the plot when there is something to say (dominant loss, reverse flow, near-sonic components, steady mass imbalance, solver advisories), each stating its numbers and selecting the element it names. Any recorded run can be overlaid on any plot (**+ Compare run…**): the same channels resolved against each run, dashed and labelled per run, each read from its own captured model and interpolated onto the plot's axis so runs on different grids still line up — the "which design was better?" comparison run history exists for. A pinned baseline overlays every plot the same way. CSV export covers the current plot or every channel, and every chart exports as PNG/SVG/CSV with provenance. Live transients build charts as steps complete; cancelled runs keep a clearly labeled partial trajectory.
...
- **Isolation & export** – each job solves a frozen, hashed snapshot of the model (later edits raise a staleness banner) and never touches the model while it runs. The variant table shows convergence and result envelopes; **Export CSV** downloads one row per variant with hash provenance; **Promote** graduates a sweep point into a saved simulation variant named after the swept value, with its run filed underneath.
...
- **LOX/RP-1 thruster (transient startup)** – the same reacting-junction thruster run transiently: LOX/RP-1 feed pressure ramps 100 → 1000 psi over 1 s and holds for another second, with per-node volumes (mass storage) and a fixed 30 kPa exhaust boundary chosen to avoid an unphysical recompression kink at the low end of the ramp; see [`docs/combustion.md`](docs/combustion.md).
...
$Y$ is the expansibility factor ($Y=1$ for liquids; isentropic $Y(r,\kappa)$ for gases, choked at $r_*$). Equivalent Bernoulli inversion when $Y=1$:
...
A network has a required default `fluid` plus optional named extras in `fluids`. A node owns its fluid (`nodes[].fluid` names an extra, or omitted means the default); a branch inherits from its endpoints and may connect two nodes only when they resolve to the **same** named fluid. Unlike fluids do not mix at an ordinary junction — the only general coupling is heat through solids (convection on opposite sides of a wall). An explicitly declared CEA reacting junction is the narrow exception: oxidizer and fuel inlet branches terminate at a chamber and produce one named ideal-gas stream. EOS **classes** may otherwise differ between continua (e.g. an `idealGas` hot side with a `realFluid` coolant): since unlike fluids never share an ordinary equation, the solver dispatches property access per node. Species transport stays a composition within one ideal gas and is rejected when named fluids are present.
...
| `junctions`                           | `JunctionConfig[]`                                                               | no                       | CEA-coupled reacting junctions (steady or transient); requires `kineticEnergy`, cannot be combined with `species`. Transient mode additionally requires the junction's node to carry a positive `volume` (mass storage; the energy closure itself stays quasi-steady — see [`docs/combustion.md`](docs/combustion.md)).              |
| `junctions[].node`                    | string                                                                           | yes                      | Internal chamber node using the same named `idealGas` as `productFluid`.                                                                                                                                                                                                                                                             |
| `junctions[].inlets`                  | `{ branch: string, role: "oxidizer" \| "fuel" }[]`                               | yes                      | Reactant branches ending at the chamber; multiple branches with one role are summed.                                                                                                                                                                                                                                                 |
| `junctions[].model`                   | `{ type: "ceaTable", propellants: "lox-rp1" \| "lox-ch4", efficiency?: number }` | yes                      | NASA CEA chamber table and enthalpy efficiency in (0, 1]; tables cover Pc 0.2–30 MPa and O/F 1–5 with edge clamping.                                                                                                                                                                                                                 |
| `junctions[].productFluid`            | string                                                                           | yes                      | Named `idealGas` entry receiving lagged CEA-derived R, γ, μ, and cp.                                                                                                                                                                                                                                                                 |
...
| `variants[].id`                       | string                                                                           | yes                      | Unique variant identifier (own namespace)                                                                                                                                                                                                                                                                                            |
| `variants[].name`                     | string                                                                           | yes                      | Display name, e.g. `"Cold day"`                                                                                                                                                                                                                                                                                                      |
| `variants[].patch`                    | object                                                                           | no                       | Sparse overrides on the base network: `settings`, `fluid`, per-id `nodes` / `branches` / `solidNodes` / `conductors` field maps, `added` entities, and `removed` ids. Absent means the variant matches the base. Never seen by the solver — resolved away first (`core/variants.ts`) and excluded from the provenance hash.          |
...
| `branches[].component.type`           | `"pipe"`                                                                         | `"orifice"`              | `"resistance"`                                                                                                                                                                                                                                                                                                                       | `"valve"`                                                                                                    | `"checkValve"`                                                                                                                                                                                            | `"dynamicCheckValve"` | `"reliefValve"` | `"pump"` | `"bend"` | `"areaChange"` | `"cavitatingVenturi"` | `"flowSource"` | `"regulator"` | `"heatedPipe"` | `"dpTable"` | `"customResistance"` | `"userComponent"` | yes | Component model |
...
**Species architecture.** Operator splitting with node-local stiff ODE integration (BDF1 + dense Newton + adaptive sub-stepping), **not** monolithic Newton coupling of species into the global `[P, mdot, h]` vector. Species transport is advective upwinding in the outer successive-substitution loop (mirrors the enthalpy update). The chemistry sub-step runs once per converged time step, updating `nodeY` and `nodeT` for each internal node. This path is transient-only. Only `idealGas` + species is valid; `realFluid` + species and `incompressible` + species are rejected by validation.

**Combustion architecture.** `config.junctions` provides a separate,
monolithically coupled CEA chamber closure for `kineticEnergy` solves, steady
or transient. It currently supports LOX/RP-1 and LOX/CH₄, requires a named
ideal-gas product fluid, and cannot be combined with species transport.
Product composition is frozen downstream and the tables assume standard-state
reactant injection. In transient mode the junction's mass balance carries a
genuine `d(ρV)/dt` storage term (so `Pc(t)` responds to a feed-pressure ramp
with real fill dynamics), while the energy closure stays quasi-steady —
combustion/residence time is far below any ramp rate a network's boundary
schedules would author, so the chamber reaches its instantaneous CEA
equilibrium enthalpy without its own energy storage. See
[`docs/combustion.md`](docs/combustion.md).
...
- **No shock capture.** With `momentumFlux` + `kineticEnergy` the solver handles quasi-1-D compressible duct flow — Fanno friction choking, Rayleigh thermal choking, and converging–diverging nozzles, validated against the NASA GFSSP verification paper (see [Compressible duct flow](#compressible-duct-flow-settingskineticenergy)). A supersonic bell is reachable when seeded onto that branch, as the shipped LOX/RP-1 thruster does. But there is no Rankine–Hugoniot jump condition, so shock position, over-expanded/separated operation, and any supersonic → subsonic transition are out of scope; thrust is not computed either, since the solver returns the internal flow field rather than a vehicle momentum balance. Point choking at restrictions is still available via `orifice` ($Y(r,\kappa)$) and `cavitatingVenturi` (choked liquid). With the flags off (the default), momentum is the incompressible per-branch relation $\Delta P(\dot{m})$ with no Fanno-flow treatment.
- **Kinetic-energy transport** – `kineticEnergy` works with any fluid model. The fully-coupled pressure–flow–enthalpy Newton system that holds near-sonic states now runs in transient mode too (real fluid via the extended `[P, ṁ, h]` system; ideal-gas/incompressible via the same coupled `[P, ṁ, h]` system steady solves use — this is also what lets a reacting junction's chamber pressure evolve transiently, see [`docs/combustion.md`](docs/combustion.md)). Only species-transport networks still take the segregated stagnation-enthalpy update, which is not designed to track choking fronts. Near-choked cases benefit from choke-clustered grids; under the default `momentumFluxScheme: "upwind"` they converge from cold starts, while `"central"` needs initial guesses (`initialMdot`, node P/T) on the physical branch, as GFSSP itself requires. Without the flag, the energy equation fluxes static enthalpy only, so velocity-head-to-temperature conversion is not modeled.
...
- **No general junction mixing** – outside an explicitly declared CEA reacting junction, unlike fluids may not meet at a node. Couple them thermally through a solid wall. Mixed EOS classes (e.g. an ideal-gas hot side with a CoolProp coolant) are otherwise allowed exactly because the wall is the only coupling.
...
- [x] Reacting flows — transient ideal-gas species transport with stiff Arrhenius chemistry and CEA-coupled reacting junctions (steady and transient) (see [Species Transport and Reacting Flows](#species-transport-and-reacting-flows))
...
- [x] Kinetic-energy (stagnation-enthalpy) term in the energy balance (`settings.kineticEnergy`, any EOS via the coupled `[P, ṁ, h]` system, steady or transient)
