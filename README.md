...
- **Rocket combustion chamber (choked CD nozzle)** – hot representative LOX/RP-1 products (γ = 1.20, 1 MPa, 3200 K) expanding through a **choked** converging–diverging nozzle with `momentumFlux` + `kineticEnergy`. Static pressure falls monotonically 987 → 43.5 kPa and Mach rises monotonically 0.15 → 2.6 through the sonic throat at ε = 4; the solver finds the choked mass flow from the pressure ratio alone — 0.802 kg/s under the default seed-robust limited-upwind faces (the documented few-percent first-order sonic-cell bias above the 0.758 kg/s analytic value; `momentumFluxScheme: "central"` recovers sub-1% given a warm start). Throat-clustered stations resolve the crossing; the shipped `initialMdot` warm start is a convenience under `"upwind"` (the same root is reached from a flat cold start) and load-bearing under `"central"`.
