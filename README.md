...
- **Persistence** – `.fn` text save/load (lossless text projection), localStorage autosave, 13 built-in examples.
...
- **Regenerative cooling channel (LOX/RP-1 booster chamber)** – one RP-1 channel in a regeneratively cooled booster jacket (Bartz-plus-soot hot side, GRCop-84 liner).
- **Rocket combustion chamber (choked CD nozzle)** – hot representative LOX/RP-1 products (γ = 1.20, 1 MPa, 3200 K) expanding through a **choked** converging–diverging nozzle with `momentumFlux` + `kineticEnergy`. Static pressure falls monotonically 987 → 43.5 kPa and Mach rises monotonically 0.15 → 2.6 through the sonic throat at ε = 4; the solver finds the choked mass flow (0.753 kg/s, within 0.7% of analytic) from the pressure ratio alone. Throat-clustered stations and an `initialMdot` warm start put the Newton on the supersonic branch.

#### Benchmarks

- **GFSSP Ex.5: Water-water counterflow HX (steady)** – steady-state solve of the published GFSSP v5 manual Example 5; the N=12 model reaches published outlet temperatures within 0.44 K (hot) and 0.19 K (cold).
- **Lee & Martin: entrapped-air (GFSSP Fig.10)** – transient entrapped-air line with fluid inertia on all pipe segments and a polytropic gas cushion at the downstream end.
- **SINDA/FLUINT: Cryogenic line cooldown (NBS 9264, Fig. 2)** – faithful N=20 reimplementation of the C&R Tech validation case (NBS Report 9264 Fig. 2, Brennan et al. 1966): saturated LH₂ at 75 psia admitted to a 61 m copper line (15.9 mm ID) initially at 300 K, venting to 0.82 atm; each of the 20 axial segments carries a copper wall thermal mass with temperature-dependent NIST OFHC-copper cp(T) and Miropolskii film-boiling convection.
...
| `settings.coupledHonestyGate`         | boolean                                                 | no                       | Experimental (default off): certify transient real-fluid steps on the post-coupling residual. See [`docs/solver-convergence.md`](docs/solver-convergence.md).                                                                                            |
