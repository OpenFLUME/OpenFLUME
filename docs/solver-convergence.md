...
> Bail honestly (`converged = false`) iff the best certifying scaled
...
- The gate is opt-in and off by default. The default path retains the pre-coupling certification-lag artifact (§1).
- Failing steps march on with best-residual compromise states. Their per-step energy non-closure (up to ~13–22 % worst-step) contaminates any gate-active energy budget. Certifiable steps close to ~0.1 % per step.
