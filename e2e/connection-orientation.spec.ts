...
test.describe("Connection orientation", () => {
  ...
  test("4. Orientation recomputes when a node is dragged vertically", async ({
    page,
  }) => {
    const consoleWatcher = attachConsoleWatcher(page);
    ...
    await page.evaluate(() => {
      localStorage.setItem(
        ...
        JSON.stringify({
          meta: { name: "Orientation drag", version: 2 },
  ...
  test("5. Group container edge uses container geometry", async ({ page }) => {
    ...
    await page.evaluate(() => {
      localStorage.setItem(
        "fluids-network-config-v1",
        JSON.stringify({
          meta: { name: "Group orientation", version: 2 },
          settings: { mode: "steady", tolerance: 1e-8, maxIterations: 60 },
          fluid: { model: "incompressible", preset: "water" },
          nodes: [
            {
    ...
    await expect(
      page.getByRole("button", { name: /Open subnetwork/ }),
    ).toBeVisible();
    await page.reload();
    await collapseAddElements(page);
  ...
  test("6. Reversed solved flow: no arrowheads, dashed run, flipped directional symbol", async ({
    ...
    await page.evaluate(() => {
      localStorage.setItem(
        "fluids-network-config-v1",
        JSON.stringify({
          meta: { name: "Reversed pipe", version: 2 },
          settings: { mode: "steady", tolerance: 1e-8, maxIterations: 60 },
          fluid: { model: "incompressible", preset: "water" },
          nodes: [
            {
    ...
    // The reversed-flow cue is the dashed run.
